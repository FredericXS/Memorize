...
struct MemoryGame<CardContent> where CardContent: Equatable {
    ...
    mutating func startCards() {
        cards.indices.forEach { index in
            cards[index].isStarted = true
        }
    }
    
    mutating func cardsFaceUpToggle() {
        cards.indices.forEach { index in
    ...
    mutating func choose(_ card: Card) {
        // Finding the index of the chosen card in the cards array.
        if let chosenIndex = cards.firstIndex(where: { $0.id == card.id }) {
            // Checks if card chosen is face down and not matched
            if !cards[chosenIndex].isFaceUp && !cards[chosenIndex].isMatched {
                
                if let potentialMatchIndex = indexOfTheOneAndOnlyFaceUpCard {
                    // If the two chosen cards have the same content, then they match
    ...
    struct Card: Equatable, Identifiable, CustomDebugStringConvertible {
        ...
        var isFaceUp = false {
            didSet {
                if isFaceUp && isStarted {
                    startUsingBonusTime()
                } else {
                    stopUsingBonusTime()
                }
                
                if oldValue && !isFaceUp && isStarted {
                    hasBeenSeen = true
                }
            }
        }
